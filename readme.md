...
## **name** if app.py is run directly then **name** == **main** but if it is imported then **name** == app
...
## Adding user specific session in Flask

    1. Add secret key to your app

    	app.secret_key = 'todolistapp'

    2. Then after successfull login, create an session as follows:

    	session['response'] = user.email_id --> assigning email_id to session so that we can fetch user data for subsequent tasks

    Note - Always assign primary key to session, it helps a lot

    3. Validating session for user

    	if 'response' in session:

    		//perform tasks

    4. Removing an session
